...
export function Slider({
  ...
  function onPointerDown(e: ReactPointerEvent) {
    ...
    // sliders); applyStyle() tags that update with SKIP_SELECTION_FOCUS_TAG so Lexical's
    // DOM-selection reconciliation doesn't refocus the contentEditable and undo the blur
    // above, so no re-blur is needed here beyond the one at drag start.
...
export function VerticalSlider({
  ...
  function onPointerDown(e: ReactPointerEvent) {
    ...
    // runs a Lexical editor.update() tagged with SKIP_SELECTION_FOCUS_TAG in
    // applyStyle(), so it no longer refocuses the contentEditable mid-drag and no
    // re-blur is needed beyond the one at drag start.
    const move = (ev: PointerEvent) => {
